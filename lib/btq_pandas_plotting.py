...
def scatter_matrix_and1(frame, alpha=0.5, figsize=None, ax=None, grid=False,
                   diagonal='hist', rowname=None, marker='.', density_kwds=None,
    ...
    if rowname is not None:
        one_df = df.loc[rowname]
        #onemask = com.notnull(one_df)
    ...
    for i, a in zip(lrange(n), df.columns):
        for j, b in zip(lrange(n), df.columns):
            ...
            if i == j:
                ...
                if rowname is not None:
                    #onevals = (onemask[a] & onemask[b]).values
                    ax.plot(one_df[b], one_df[a],
                           marker=marker, alpha=.8, color='k',
                           markersize=10)
